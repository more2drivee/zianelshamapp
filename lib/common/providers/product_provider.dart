...
Future<void> getLatestProductList(int offset, bool reload, { bool isUpdate = true}) async {
  ...
  if (reload) {
    ...
    print("🔄 Reloading products, clearing old data...");

    if (isUpdate) {
      print("📢 notifyListeners() after clearing data");
      notifyListeners();
    }
  }

  if (offset == 1) {
    print("⬇️ Fetching products (offset = 1)...");
    fetchAndSyncData(
      fetchFromLocal: () => productRepo!.getLatestProductList<CacheResponseData>(
        offset: offset,
        type: productSortProvider.selectedShotType,
        source: DataSourceEnum.local,
      ),
      fetchFromClient: () => productRepo!.getLatestProductList(
        offset: offset,
        type: productSortProvider.selectedShotType,
      ...
      onResponse: (data, _) {
        ...
        print("✅ Products loaded from fetchAndSyncData: ${_latestProductModel?.products?.length}");
        notifyListeners();
      },
    );
  } else {
    if (_latestProductModel == null || offset != 1) {
      print("⬇️ Fetching products (offset = $offset) from API...");
      ApiResponseModel? response = await productRepo?.getLatestProductList(
        offset: offset,
        type: productSortProvider.selectedShotType,
        source: DataSourceEnum.client,
      );
      ...
      if (response?.response?.data != null && response?.response?.statusCode == 200) {
        if (offset == 1) {
          ...
          print("✅ Products loaded from API (offset=1): ${_latestProductModel?.products?.length}");
        } else {
          final newData = ProductModel.fromJson(response?.response?.data);
          _latestProductModel?.totalSize = newData.totalSize;
          _latestProductModel?.offset = newData.offset;
          _latestProductModel?.products?.addAll(newData.products ?? []);
          print("✅ More products appended, total now: ${_latestProductModel?.products?.length}");
        }

        notifyListeners();
      } else {
        print("❌ API returned error or empty response");
