...
class CategoryProvider extends DataSyncProvider {
  ...
  // store subcategories per category id so each accordion can show its own list
  List<CategoryModel>? _subCategoryList;
  final Map<String, List<CategoryModel>> _subCategoryMap = {};
  ...
  List<CategoryModel>? get subCategoryList => _subCategoryMap[_selectedSubCategoryId] ?? _subCategoryList;
bool get isCategorySelected => _isCategorySelected; // Getter علشان نستعمله في الـ UI

  // return subcategories for a specific category id (may be null if not fetched)
  List<CategoryModel>? getSubCategoriesFor(String categoryID) => _subCategoryMap[categoryID];
  ...
  Future<void> getCategoryList(bool reload, {DataSourceEnum source = DataSourceEnum.local, int limit = 500, int offset  =1 }) async {
  ...
  _isCategorySelected = true;   // ✅ أول ما يختار كاتيجوري بنفعل الفلاج
  _isLoading = true;
  notifyListeners();
    // mark this category as selected so getter returns its subcategories
    // show loading for this fetch
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      apiResponse.response!.data.forEach((category) => fetched.add(CategoryModel.fromJson(category)));
      // save into map for this category id
      _subCategoryMap[categoryID] = fetched;
      // keep legacy field for backward compatibility
      _subCategoryList = fetched;
      // fetch products for this category (will reuse selectedSubCategoryId)
  ...
  _isCategorySelected = false;   // ✅ نرجع الفلاج False
  _selectedSubCategoryId = null; // ونفضي الـ ID
  notifyListeners();
}
}
